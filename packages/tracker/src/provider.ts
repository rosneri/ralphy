...
/**
 * The single seam every issue-tracker backend implements. Promoted from a
 * latent provider interface that once lived in the test harness (RLF-223 M1):
 * the coordinator and wire layer depend only on this shape, never on a concrete
 * transport. `LinearTrackerProvider` wraps the existing Linear GraphQL client;
 * `GithubTrackerProvider` implements the same bag over `gh`.
 *
