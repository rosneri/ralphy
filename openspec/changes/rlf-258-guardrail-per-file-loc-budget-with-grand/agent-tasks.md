## Resolve PR merge conflicts (2026-06-17T11:57:08.574Z)
...
4. Push the resolved branch with `git push origin ralph/rlf-258`. Never force-push.
   ...
   If the push is rejected, inspect the rejection output and react inline before retrying:
     - **non-fast-forward** (someone else pushed to `ralph/rlf-258`):
       `git fetch origin ralph/rlf-258` then `git merge origin/ralph/rlf-258` to bring their
...
PR: https://github.com/rosneri/ralphy/pull/449
```

## Fix failing CI checks (2026-06-17T00:06:15.222Z)
...
Steps:
1. Inspect the failing checks: `gh pr checks https://github.com/rosneri/ralphy/pull/449` then
...
4. Push with `git push origin ralph/rlf-258`. If the push is rejected as
   non-fast-forward, `git fetch origin ralph/rlf-258` then `git merge origin/ralph/rlf-258`
...
## Resolve PR merge conflicts (2026-06-16T23:59:08.565Z)
...
## Fix failing CI checks (2026-06-15T21:28:18.854Z)
...
## Resolve PR merge conflicts (2026-06-15T21:05:58.362Z)
