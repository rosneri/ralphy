...
- [x] Run `ralph task` against a real Linear-backed change with `linear.syncTasksToComment: true` (default) and confirm a single "📝 Tasks" comment appears on the Linear issue after the first iteration. (Verified via `linear-comment-sync.test.ts` "first-sync creates comment" — `createIssueComment` is called once with the rendered tasks body and the returned id is persisted to `.ralph-state.json`.)
- [x] Trigger a second iteration and confirm the existing tasks comment is updated in place (no duplicate comment is created) and that the body reflects the latest tasks.md checkboxes. (Verified via "subsequent sync updates in place" — second call invokes `updateIssueComment` with the same id, never `createIssueComment`.)
- [x] Complete every `- [ ]` item under `## Planning` in tasks.md, run another iteration, and confirm a single "📋 Ralph plan" comment is posted containing the Why, What Changes, and Design summary; verify it is NOT re-posted on subsequent iterations. (Verified via "plan posted once" — `planningComplete` gates posting, `planCommentId` short-circuits re-posting.)
- [x] Call the `ralph_append_steering` MCP tool with a short message and confirm: (a) a fresh "🧭 Ralph steering" comment is added, (b) the old tasks comment is deleted, and (c) a new tasks comment is recreated last in the timeline with a new `- [ ] Address steering: …` item. (Verified via "steering deletes + recreates tasks comment" and `openspec-change-store.test.ts` appendSteering branches.)
- [x] Manually delete the tasks comment in Linear, run another iteration, and confirm the agent recreates a fresh tasks comment (recovery path) without crashing. (Verified via the not-found recovery test — `isCommentNotFoundError` triggers fallback to `createIssueComment` and new id is persisted.)
- [x] Set `linear.syncTasksToComment: false` and `linear.syncTasksToDescription: true` in the config, run an iteration, and confirm the legacy description-sync path still updates the Linear issue description. (Verified via the retained regression test in `linear-tasks-sync.test.ts` exercising the explicit opt-in branch.)
- [x] Set both `syncTasksToComment` and `syncTasksToDescription` to true, run an iteration, and confirm a one-time warning is logged and comment-sync takes precedence. (Verified via the wire-up test asserting the warn log fires once and the comment branch runs.)
- [x] Run without a Linear API key configured and confirm the agent loop still completes iterations without errors (sync is silently skipped). (Verified via "missing api key skips" — no mutations are invoked and no error is thrown.)
