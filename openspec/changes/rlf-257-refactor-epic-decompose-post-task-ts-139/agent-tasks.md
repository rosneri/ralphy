## Fix failing CI checks (2026-06-17T12:44:54.243Z)
...
Steps:
1. Inspect the failing checks: `gh pr checks https://github.com/rosneri/ralphy/pull/439` then
...
4. Push with `git push origin ralph/rlf-257`. If the push is rejected as
   non-fast-forward, `git fetch origin ralph/rlf-257` then `git merge origin/ralph/rlf-257`
...
PR: https://github.com/rosneri/ralphy/pull/439
```
...
## Fix failing CI checks (2026-06-17T12:09:32.942Z)
...
## Fix failing CI checks (2026-06-17T12:03:26.929Z)
...
## Fix failing CI checks (2026-06-17T00:06:00.075Z)
...
4. Push the resolved branch with `git push origin ralph/rlf-257`. Never force-push.
   ...
   If the push is rejected, inspect the rejection output and react inline before retrying:
     - **non-fast-forward** (someone else pushed to `ralph/rlf-257`):
       `git fetch origin ralph/rlf-257` then `git merge origin/ralph/rlf-257` to bring their
...
## Fix failing CI checks (2026-06-15T21:27:38.313Z)
...
## Resolve PR merge conflicts (2026-06-15T20:40:32.843Z)
...
## Resolve PR merge conflicts (2026-06-14T06:36:22.106Z)
