...
- [x] Verify `phasePipeline("review")` shows review as current: run `bun -e "import { phasePipeline } from './packages/core/src/openspec/phase.ts'; console.log(JSON.stringify(phasePipeline('review').map(s => s.phase + ':' + s.status)))"` and confirm review has status `current` and all earlier phases have status `done`.
...
- [x] Verify `buildTaskPrompt` injects the address-findings block when review-findings.md has open items: run `bun test packages/core/src/__tests__/loop.test.ts --reporter=verbose 2>&1 | grep -E "(address|findings|PASS|FAIL)"` and confirm the address-findings test case passes (all 40 tests pass).
- [x] Verify review-self.md prompt contains all required sections: run `bun -e "const t = await Bun.file('./packages/content/src/review-self.md').text(); console.log(['proposal.md','design.md','tasks.md','git diff','review-findings.md','## Open','(no findings — close round)'].every(s => t.includes(s)))"` and confirm it prints `true`.
- [x] Run `bunx openspec validate review-phase` and confirm it exits 0 — validates the review-phase spec created for this change.
